...
<br><br><br>
		<form method="post" class="form-register" enctype="multipart/form-data">
				...
				<div class="contenedor-inputs">
					<input type="text" name="name" value="<?=$nombre;?>" placeholder="Nombre" class="input-48" required>
					<?php if (isset($erroresFinales['nombre'])): ?>
						<span class="error"><?=$erroresFinales['nombre'];?></span>
					<?php endif; ?>

					<input type="text" name="apellido" value="<?=$apellido;?>" placeholder="Apellido" class="input-48"required>
					<?php if (isset($erroresFinales['apellido'])): ?>
						<span class="error"><?=$erroresFinales['apellido'];?></span>
					<?php endif; ?>

					<input type="text" name="email" value="<?=$email;?>" placeholder="Correo Electronico" class="input-100"required>
					<?php if (isset($erroresFinales['email'])): ?>
						<span class="error"><?=$erroresFinales['email'];?></span>
					<?php endif; ?>

					<input type="text" name="username" value="<?=$username;?>" placeholder="Usiario" class="input-100"required>
